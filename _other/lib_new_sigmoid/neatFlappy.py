...
def createPipe(dist, SCREEN_HEIGHT, BIRD_HEIGHT, PIPE_WIDTH):
...
def runFlappy(net):
    ...
    dist = SCREEN_WIDTH
    for i in range(0,5):
        pipes.append(createPipe(dist + PIPE_GAP* i, SCREEN_HEIGHT,  BIRD_HEIGHT, PIPE_WIDTH))
    ...
    while 1:
        ...
        if pipes[0][0].x <= 0:
            ...
            pipes.append(createPipe(pipes[len(pipes) - 1][0].x + PIPE_GAP, SCREEN_HEIGHT, BIRD_HEIGHT, PIPE_WIDTH))
