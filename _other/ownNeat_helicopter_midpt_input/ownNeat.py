...
class Bird(object):
	...
	def __init__(self,pos_x,pos_y,genome):
		...
		self.height = None
		self.nextPipeTop = None
		self.nextPipeBottom = None
	...
	def setSprite(self, spriteNo):
		self.birdImg = "b"+str(spriteNo)+".png"
		tempBird = pygame.image.load(birdimg)
		self.drawBird = pygame.transform.scale(tempBird, (BIRD_WIDTH, BIRD_HEIGHT))
		self.rect = self.drawBird.get_rect()
	...
	def checkCollision(self,screen_height,pipes):
		...
		for pipe in pipes:
			if pygame.Rect.colliderect(self.rect,pipe.rect):
				self.isAlive = False
				return False
	...
	def setInputs(self, pipes):
		...
		for pipe in pipes:
			if pipe.rect.x <= self.rect.x:
				...
				self.nextPipeTop = pipe.rect.y
				self.nextPipeBottom = pipe.rect.y + OBS_HEIGHT
				break
...
class Pipe(object):
	...
	def __init__(self,pos_x,pipeid):
		...
		midpt = random.randint(0 - OBS_HEIGHT , SCREEN_HEIGHT)

		self.rect = pygame.Rect(pos_x,midpt,PIPE_WIDTH, OBS_HEIGHT)
	...
	while not allDead:
		...
		for bird in birds:
			if bird.isAlive == True:
				# Calculate score
				for pipe in pipes:
					if pipe.rect.x <= bird.iniPosX and bird.hasCrossed(pipe.id) == False:
		...
		if pipes[0].rect.x <= 0:
			...
			pipes.append(Pipe(pipes[len(pipes) - 1].rect.x + PIPE_GAP, pipes[len(pipes) - 1].id + 1))
		...
		for bird in birds:
			if bird.isAlive == True:
				# obs_height = (bird.nextPipeTop - bird.nextPipeBottom)
				top_gap = bird.nextPipeTop
				bottom_gap = SCREEN_HEIGHT - bird.nextPipeBottom
				topinp = top_gap / 2
				bottominp = bird.nextPipeBottom + bottom_gap/2
				output = bird.genome.evaluateNetwork([bird.height, bird.nextPipeTop + (bird.nextPipeBottom - bird.nextPipeTop)/2, 1])
				if output[0] > 0.5:
					bird.isJumping = True
